# -*- coding: utf-8 -*-
"""
WNBA球队十年收益预测（最终优化版+Clark交易逻辑）
核心新增：
1. Clark连年受伤触发交易逻辑（连续2年受伤+至少1次中重度）
2. 定义Clark替代球员（能力稍弱但接近）
3. 交易前后数据对比，展示球队回升效果
4. 可视化标注交易节点，体现回升趋势
"""
...
# ===============================
# 2. 球队类：控制伤病人数+强化Clark影响（原有代码完全保留）
# ===============================
class Team:
...
# ===============================
# 新增：定义Clark替代球员（能力稍弱但接近）
# ===============================
def create_clark_replacement(current_year):
    """
    创建Clark的替代球员（能力稍低于Clark，但差距不大）
    current_year: 交易年份，用于匹配年龄
    """
    # Clark原始属性：off=95, deff=68, fame=99, salary=76535, age=22（2023年）
    # 替代球员属性：能力稍弱（off93, deff66），知名度稍低（92），薪资相近，年龄匹配
    replacement_age = 22 + (current_year - 2023)  # 匹配Clark的年龄
    return Player(
        name="Replacement_Clark",
        off=93,  # 比Clark低2点
        deff=66,  # 比Clark低2点
        fame=92,  # 比Clark低7点
        salary=78000,  # 略高于Clark（76535）
        age=replacement_age
    )


# ===============================
# 5. 十年预测主循环：新增Clark交易逻辑（核心新增）
# ===============================
def run_forecast():
    # 初始阵容
    roster = [
    ...
    # 新增：Clark伤病跟踪&交易标记
    clark_injury_years = []  # 记录Clark受伤的年份
    trade_triggered = False  # 是否触发交易
    trade_year = None  # 交易年份
    pre_trade_data = {}  # 交易前数据
    post_trade_data = {}  # 交易后数据
    clark_replacement = None  # 替代球员
    ...
    for y in range(1, 11):
        ...
        year_data = {
            ...
            "Trade_Triggered": False  # 新增：标记是否交易
        }
        history.append(year_data)

        # ===============================
        # 新增：Clark连年受伤检测&交易逻辑
        # 触发条件：连续2年受伤 + 至少1次中重度伤病
        # ===============================
        # 记录Clark受伤年份
        if team.clark_injured:
            clark_injury_years.append(current_year)
            # 检查是否连续2年受伤
            if len(clark_injury_years) >= 2 and not trade_triggered:
                # 检查是否有中重度伤病
                clark_injury_detail = [inj for inj in yearly_injuries if inj["is_clark"]]
                if clark_injury_detail and clark_injury_detail[0]["level"] >= 2:
                    # 触发交易
                    trade_triggered = True
                    trade_year = current_year
                    print(f"\n=== 触发Clark交易 ===")
                    print(f"交易年份：{trade_year}")
                    print(
                        f"触发原因：Clark连续{len(clark_injury_years)}年受伤，且本次为{clark_injury_detail[0]['level_desc']}伤病")
                    ...
                    pre_trade_data = {
                        ...
                        "Brand": brand,
                        "Wins": team.wins,
                        "Clark_Injury_Level": clark_injury_detail[0]["level_desc"]
                    }

                    # 替换Clark：移除原Clark，添加替代球员
                    roster = [p for p in roster if not p.is_clark]
                    clark_replacement = create_clark_replacement(current_year)
                    roster.append(clark_replacement)
                    print(f"交易详情：移除Caitlin Clark，新增替代球员Replacement_Clark")
                    print(
                        f"替代球员属性：off={clark_replacement.off}, deff={clark_replacement.deff}, fame={clark_replacement.fame}, salary={clark_replacement.salary}, age={clark_replacement.age}")

                    # 标记交易年份
                    history[-1]["Trade_Triggered"] = True
    ...
    # 新增：记录交易后数据（对比回升效果）
    # ===============================
    if trade_triggered and trade_year:
        # 找到交易后第一年的数据
        post_trade_idx = next((i for i, h in enumerate(history) if h["Year"] == trade_year + 1), None)
        if post_trade_idx is not None:
            post_trade_data = history[post_trade_idx]
            # 计算回升幅度
            profit_recovery = post_trade_data["Profit_M"] - pre_trade_data["Profit_M"]
            brand_recovery = post_trade_data["Brand"] - pre_trade_data["Brand"]
            wins_recovery = post_trade_data["Wins"] - pre_trade_data["Wins"]
            print(f"\n=== 交易后效果对比（{trade_year} vs {trade_year + 1}）===")
            print(
                f"利润变化：{pre_trade_data['Profit_M']:.2f} M → {post_trade_data['Profit_M']:.2f} M | 回升{profit_recovery:.2f} M")
            print(
                f"品牌值变化：{pre_trade_data['Brand']:.1f} → {post_trade_data['Brand']:.1f} | 回升{brand_recovery:.1f}")
            print(f"胜场变化：{pre_trade_data['Wins']} → {post_trade_data['Wins']} | 回升{wins_recovery}场")
    ...
    return (pd.DataFrame(history), injury_records, pd.DataFrame(injury_summary),
            trade_triggered, trade_year, pre_trade_data, post_trade_data, clark_replacement)


# ===============================
# 6. 可视化：新增交易节点标注（核心修改）
# ===============================
def plot_injury_profit_analysis(df, injury_records, trade_triggered, trade_year):
    ...
    for idx, row in df.iterrows():
        ...
        # 新增：标注交易节点
        if row["Trade_Triggered"]:
            injury_text += "\n【触发交易：替换Clark】"
            # 高亮交易节点
            ax1.scatter(year, profit, color='green', s=200, marker='*', zorder=10,
                        label="交易节点" if idx == df[df['Trade_Triggered']].index[0] else "")
    ...
    # 新增：交易后回升标注
    if trade_triggered and trade_year:
        post_trade_year = trade_year + 1
        if post_trade_year in df['Year'].values:
            post_trade_profit = df[df['Year'] == post_trade_year]['Profit_M'].values[0]
            pre_trade_profit = df[df['Year'] == trade_year]['Profit_M'].values[0]
            # 绘制回升箭头
            ax1.annotate(
                f"交易后回升\n{pre_trade_profit:.2f}→{post_trade_profit:.2f} M",
                xy=(post_trade_year, post_trade_profit),
                xytext=(-30, 30),
                textcoords='offset points',
    ...
    ax1.set_title("WNBA球队十年利润/品牌趋势（含Clark交易节点）", fontsize=16, pad=20)
    plt.tight_layout()
    plt.savefig("profit_injury_trend_with_trade.png", dpi=300, bbox_inches='tight')
    plt.show()

    # 子图2：核心伤病vs利润（新增Clark交易标注）
    ...
    # 交易节点高亮
    if trade_triggered and trade_year:
        trade_row = df[df['Year'] == trade_year]
        ax.scatter(
            trade_row["Core_Injury_Count"], trade_row["Profit_M"],
            color='green', s=300, marker='*', zorder=10, label="交易节点"
        )
    ...
    for idx, row in df.iterrows():
        annot_text = str(row["Year"]) + ("(Clark伤)" if row["Clark_Injured"] == 1 else "")
        if row["Trade_Triggered"]:
            annot_text += "\n(交易)"
        ax.annotate(
            annot_text,
    ...
    ax.set_title("核心球员（含Clark）伤病数与年度利润的关联分析（含交易）", fontsize=16, pad=20)
    ax.grid(alpha=0.3)
    ...
    legend_elements = [
        ...
        Patch(facecolor='green', label='交易节点')
    ]
    ax.legend(handles=legend_elements + ax.get_legend_handles_labels()[0], loc='upper right')

    plt.tight_layout()
    plt.savefig("core_injury_profit_correlation_with_trade.png", dpi=300, bbox_inches='tight')
    plt.show()


def plot_3d_metrics(df, trade_triggered, trade_year):
    ...
    # 交易节点高亮
    if trade_triggered and trade_year:
        trade_idx = df[df['Year'] == trade_year].index[0]
        ax1.scatter(x1[trade_idx], y1[trade_idx], z1[trade_idx],
                    color='green', s=300, marker='*', zorder=10, label="Trade Year")
    ...
    if trade_triggered and trade_year:
        trade_idx = df[df['Year'] == trade_year].index[0]
        ax2.scatter(x2[trade_idx], y2[trade_idx], z2[trade_idx],
                    color='green', s=300, marker='*', zorder=10)
    ...
    if trade_triggered and trade_year:
        trade_idx = df[df['Year'] == trade_year].index[0]
        ax3.scatter(x3[trade_idx], y3[trade_idx], z3[trade_idx],
                    color='green', s=300, marker='*', zorder=10)
    ...
    plt.suptitle("3D Visualization: Key Metrics vs Profit vs Time (10-Year Forecast) + Trade", fontsize=16)
    plt.tight_layout()
    plt.show()
...
# 新增：交易前后对比分析函数
# ===============================
def analyze_trade_impact(pre_trade_data, post_trade_data, clark_replacement):
    """分析交易Clark对球队的影响"""
    if not pre_trade_data or not post_trade_data:
        print("\n=== 未触发Clark交易 ===")
        print("原因：Clark未满足「连续2年受伤+至少1次中重度」条件")
        return

    print("\n" + "=" * 100)
    print("Clark交易前后核心指标对比分析")
    print("=" * 100)

    # 输出替代球员数据
    print("\n【替代球员详细数据】")
    print("-" * 80)
    print(f"球员姓名：{clark_replacement.name}")
    print(f"进攻能力：{clark_replacement.off}（原Clark：95）")
    print(f"防守能力：{clark_replacement.deff}（原Clark：68）")
    print(f"知名度：{clark_replacement.fame}（原Clark：99）")
    print(f"薪资：${clark_replacement.salary:,}（原Clark：${76535:,}）")
    print(f"年龄：{clark_replacement.age}（与原Clark同龄）")

    # 输出交易前后对比
    print("\n【交易前后球队表现对比】")
    print("-" * 80)
    print(f"对比维度       | 交易前（{pre_trade_data['Year']}） | 交易后（{post_trade_data['Year']}） | 变化量    | 变化率")
    print(
        f"利润（百万$）  | {pre_trade_data['Profit_M']:>10.2f} | {post_trade_data['Profit_M']:>10.2f} | {post_trade_data['Profit_M'] - pre_trade_data['Profit_M']:>8.2f} | {(post_trade_data['Profit_M'] / pre_trade_data['Profit_M'] - 1) * 100:>6.1f}%")
    print(
        f"品牌值         | {pre_trade_data['Brand']:>10.1f} | {post_trade_data['Brand']:>10.1f} | {post_trade_data['Brand'] - pre_trade_data['Brand']:>8.1f} | {(post_trade_data['Brand'] / pre_trade_data['Brand'] - 1) * 100:>6.1f}%")
    print(
        f"胜场数         | {pre_trade_data['Wins']:>10d} | {post_trade_data['Wins']:>10d} | {post_trade_data['Wins'] - pre_trade_data['Wins']:>8d} | {(post_trade_data['Wins'] / pre_trade_data['Wins'] - 1) * 100:>6.1f}%")

    # 核心结论
    print("\n【核心结论】")
    print("-" * 80)
    profit_change = post_trade_data['Profit_M'] - pre_trade_data['Profit_M']
    if profit_change > 0:
        print(
            f"✅ 交易Clark后球队利润回升{profit_change:.2f}百万美元，回升率{(profit_change / pre_trade_data['Profit_M']) * 100:.1f}%")
        print(
            f"✅ 品牌值回升{post_trade_data['Brand'] - pre_trade_data['Brand']:.1f}，胜场提升{post_trade_data['Wins'] - pre_trade_data['Wins']}场")
        print(f"✅ 尽管替代球员能力稍弱，但消除了Clark连年伤病的负面影响，球队表现显著回升")
    else:
        print(f"⚠️  交易后利润略有下降{abs(profit_change):.2f}百万美元，建议调整替代球员策略")

    print("=" * 100)


# ===============================
# 主函数：输出伤病史统计+交易分析+可视化
# ===============================
if __name__ == "__main__":
    ...
    # 运行预测（新增交易相关返回值）
    df, injury_records, injury_summary, trade_triggered, trade_year, pre_trade_data, post_trade_data, clark_replacement = run_forecast()

    # 可视化（传入交易标记）
    plot_injury_profit_analysis(df, injury_records, trade_triggered, trade_year)
    plot_3d_metrics(df, trade_triggered, trade_year)

    # 输出核心统计
    print("\n" + "=" * 80)
    print("WNBA球队十年伤病-利润核心统计（最终优化版+Clark交易）")
    print("=" * 80)
    ...
    print(f"是否触发Clark交易：{'是' if trade_triggered else '否'}")
    if trade_triggered:
        print(f"交易年份：{trade_year}")
    ...
    # 新增：输出交易影响分析
    analyze_trade_impact(pre_trade_data, post_trade_data, clark_replacement)